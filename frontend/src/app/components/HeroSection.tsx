...
export function HeroSection({ onScrollTo }: HeroSectionProps) {
  ...
  useEffect(() => {
    ...
    // mousemove 的触发频率远高于渲染频率。原先每次移动都直接 setState，
    // 会让整个 Hero（500+ 行、上百个内联 style 对象）跟着重渲染。
    // 用 rAF 把一帧内的多次移动合并成一次更新。
    let frame = 0;
      ...
      >
        ...
        >
          <video
            ...
            loop
            playsInline
            // 这是仓库里最大的单文件（约 19MB）。preload 改为 none，并等首屏
            // 过渡结束（loaded）后再挂 src，避免它和首屏关键资源抢带宽。
            // 根本解法是压到 2~3MB 并迁到对象存储/CDN，见 docs/AUDIT.md。
            preload="none"
            src={loaded ? HERO_ARTIFACT_VIDEO : undefined}
